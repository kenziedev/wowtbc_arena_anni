...
## 2026-03-17 — 공식 PvP 리더보드 API 전환

### 공식 순위표 API 적용

- Blizzard `pvp-leaderboard` API가 Classic Anniversary에서 정상 동작하는 것을 확인
- 리더보드 생성 기준을 기존 `sources.json -> 길드 로스터 -> 개별 캐릭터 PvP 조회` 방식에서 공식 PvP 리더보드 API 기준으로 전환
- `scripts/fetch_leaderboard.py`가 현재 시즌을 조회한 뒤 `2v2`, `3v3`, `5v5` 공식 순위표를 직접 수집하도록 변경
- 순위는 로컬 재계산 대신 공식 API가 내려주는 `rank` 값을 그대로 사용

### 상세 페이지 보강 유지

- 순위표에 포함된 캐릭터만 추가 프로필 조회하여 상세 페이지 데이터 품질 유지
- 기존과 동일하게 `all_characters.json`에 직업, 종족, 길드, 진영, 특성, 장비, 아바타 정보 저장
- Supabase `characters`, `rating_snapshots` 적재 구조는 유지하여 상세 차트와 이력 화면이 계속 동작하도록 구성

### 메타 및 데이터 의미 변경

- `meta.json`의 수집 기준이 등록 길드 스캔 수가 아니라 공식 리더보드 기준 캐릭터 수로 변경
- `namespace`, `season_id`, `source` 등 공식 API 기준 메타 정보 추가
- 첫 전환 이후에는 기존 부분 집계 데이터와 공식 전체 순위 간 차이로 `rd`, `rkd` 변동폭이 크게 보일 수 있음
